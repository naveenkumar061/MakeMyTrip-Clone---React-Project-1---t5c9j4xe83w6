...
import FilghtFilterTiming from "../flightsfilter/FilghtFilterTiming";
...
function FlightsFilter({
  ...
  // Function to find the minimum price based on departure time duration
  function minPriceByDuration(list, minDuration, maxDuration) {
    if (!list) return 0;
    const filteredFlights = list.filter((flight) => {
      const exactTime = flight.departureTime;
      let time = 0;
      const [hours, minutes] = exactTime.split(":");
      time = Number(hours) + Number(minutes) / 60;
      return time >= minDuration && time <= maxDuration;
    });
  ...
  const lowestMorningDeparturePrice = minPriceByDuration(flightList, 6, 12);
  const lowestLateDeparturePrice = minPriceByDuration(flightList, 18, 24);
  const lowestAfternoonDeparturePrice = minPriceByDuration(flightList, 12, 18);
  const lowestEarlyDeparturePrice = minPriceByDuration(flightList, 0, 6);
  ...
  const popularFilters = [
    ...
    { name: "Morning Departures", price: lowestMorningDeparturePrice },
    { name: "Late Departures", price: lowestLateDeparturePrice },
    { icon: 1, name: "Air India Airline", price: lowestAirIndiaFlightPrice },
    { name: "Afternoon Departures", price: lowestAfternoonDeparturePrice },
    { name: "1 Stop", price: lowestSingleStopPrice },
    { name: "Early Morning Departures", price: lowestEarlyDeparturePrice },
  ];
  ...
  return (
    ...
    >
      ...
      <div className="pb-8">
        <h1 className="pb-2 text-lg font-semibold">
          Departure From {fromCurrentCity}
        </h1>
        <FilghtFilterTiming
          filterItems={filterItems}
          onAddFilter={onAddFilter}
