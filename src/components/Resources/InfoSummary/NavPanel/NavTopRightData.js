// Importing necessary icons
import { BsFillSuitcaseFill } from "react-icons/bs"; // Bootstrap icons
import { FaAngleDown } from "react-icons/fa"; // Font Awesome icons
import { IoBagSharp } from "react-icons/io5"; // Ionicons
import { LuFileKey } from "react-icons/lu"; // Lucide icons

// Defining an array of objects for top-right menu items
export const navTopRightData = [
  {
    // List your property item
    component: "selectableItem",
    navigateTo: "/yettocome",
    icon: (
      <LuFileKey className="h-8 w-8 rounded-full border border-gray-700 bg-stone-600 fill-amber-400 p-1 text-lg text-black" />
    ),
    title: "List your property",
    subtitle: "Start earning today!",
  },
  {
    // Introducing myBiz item
    component: "selectableItem",
    navigateTo: "/yettocome",
    icon: <IoBagSharp className="h-6 w-6 fill-orange-500" />,
    title: "Introducing myBiz",
    subtitle: "Business Travel Solution",
  },
  {
    // My Trips item
    component: "selectableItem",
    navigateTo: "/login",
    icon: <BsFillSuitcaseFill className="h-6 w-6 fill-amber-400" />,
    title: "My Trips",
    subtitle: "Manage your bookings",
  },
  {
    // Login or Create Account dropdown item
    component: "dropDownMenuItem",
    navigateTo: "/login",
    icon: <FaAngleDown className="h-4" />,
    title: "my",
    subtitle: "Login or Create Account",
  },
];
